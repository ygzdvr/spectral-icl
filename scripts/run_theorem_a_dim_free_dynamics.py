...
def _mean_diag(M: Tensor) -> Tensor:
    return torch.mean(torch.diagonal(M, offset=0))
...
def make_powerlaw_spec_and_wstar(
    ...
    *,
    ...
    """Reconstruct the original power-law problem used by run_dim_free_dynamics.

    This matches the unnormalized power-law construction used throughout the
    original repo for spectral/rotated experiments:

        spec_k = k^{-alpha}
        w*_k ∝ sqrt(k^{-alpha * beta - 1} / spec_k)

    followed by normalization so that sum_k spec_k * (w*_k)^2 = 1.
    """
...
# -----------------------------------------------------------------------------
# Exact reduced-operator extraction for the dimension-free frozen-embedding model
# -----------------------------------------------------------------------------

def extract_gamma_dim_free(
    params_tr: list[Tensor],
    Wy: Tensor,
    *,
    beta_model: float,
) -> dict[str, Tensor]:
    """Extract the exact reduced feature-space operator for model_eval_dim_free.

    For the dimension-free frozen-embedding model,

        hx = X W_x^T,
        q  = hx W_q^T,
        k  = hx W_k^T,
        v  = hy W_v^T,
        hy <- hy - (beta_model / L) * (A_masked @ v) / P_tr,

    with A = k q^T and out = hy @ W_y.

    If u = W_y and W_v^T u = alpha_v u, then the network is exactly equivalent
    to a reduced predictor with

        Gamma_exact = beta_model * alpha_v * W_x^T W_k^T W_q W_x.

    To compare against the original four-variable "dimension-free" theory, we
    also form the normalized scalar

        gamma_eff_norm = gamma_eff / sqrt(d),

    because the original script compares the learned W_v against a theory
    variable normalized by sqrt(d). Equivalently,

        gamma_fact = beta_model * (w_x^2 * w_q * w_k * w_v_norm),
        w_v_norm = alpha_v / sqrt(d).
    """
    ...
    Gamma = beta_model * alpha_v * (W_x.T @ Wk.T @ Wq @ W_x)

    gamma_eff = torch.trace(Gamma) / d
    gamma_eff_norm = gamma_eff / math.sqrt(d)
    ...
    wq = _mean_diag(Wq)
    wk = _mean_diag(Wk)
    wv_norm = alpha_v / math.sqrt(d)
    gamma_fact = beta_model * (wx**2) * wq * wk * wv_norm
...
def reduced_predictor_dim_free(
    Gamma: Tensor,
    X: Tensor,
    y: Tensor,
    *,
    ...
    """Exact reduced predictor matching model_eval_dim_free output convention.

    The returned tensor matches the *network output convention* of
    model_eval_dim_free: on test positions it is the negative prediction used in
    ...
    the train-token outputs obey

        h^{ell+1} = (I - K_tr / (L P_tr)) h^ell,
        h^0 = y_tr,

    and the test-token outputs are
...
def audit_dim_free_model(
    ...
    *,
    ...
    """Compute theorem-A diagnostics on a fixed rotated debug batch."""
    with torch.no_grad():
        out_net, _, _ = model_eval_dim_free(
            params_tr,
            Wy,
            X_dbg,
            y_dbg,
        ...
        gamma_info = extract_gamma_dim_free(params_tr, Wy, beta_model=beta_model)
        ...
        out_gamma = reduced_predictor_dim_free(Gamma, X_dbg, y_dbg, L=L, P_test=P_test)
        out_scalar = reduced_predictor_dim_free(Gamma_scalar, X_dbg, y_dbg, L=L, P_test=P_test)
        ...
        E_gamma_fact = torch.abs(gamma_info["gamma_eff_norm"] - gamma_info["gamma_fact"]) / torch.maximum(
            torch.abs(gamma_info["gamma_eff_norm"]), torch.abs(gamma_info["gamma_fact"])
        ).clamp_min(eps)
        ...
        result: dict[str, Any] = {
            ...
            "gamma_eff_norm": float(gamma_info["gamma_eff_norm"].cpu()),
            ...
            "wv_norm": float(gamma_info["wv_norm"].cpu()),
        }
        if store_outputs:
...
def train_model_dim_free_with_theorem_a(
    *,
    d: int,
    P_tr: int,
    P_test: int,
    ...
    X_dbg, y_dbg = sample_data_spec_rotate_bernoulli(
        spec,
        w_star,
        debug_batch,
        P_tr,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Theorem-A robustness experiment for dimension-free linear-attention "
            "dynamics on rotated Bernoulli spectral data. This is a non-isotropic "
            "robustness check, not the true RRS theorem experiment."
        )
    )
    ...
    parser.add_argument("--alpha", type=float, default=1.25, help="power-law spectrum exponent")
    parser.add_argument("--beta", type=float, default=1.2, help="source exponent controlling theory slope")
    ...
    d = args.d
    N = d  # the original dim-free experiment uses N = d
    T = args.steps
    sqrt_d = math.sqrt(d)
    ...
    X_chk, y_chk = sample_data_spec_rotate_bernoulli(
        spec,
        w_star,
        args.batch,
        args.p_train,
    ...
    print(f"mean(y^2) = {float(torch.mean(y_chk**2).cpu())}")

    # ------------------------------------------------------------------
    # Train the full dimension-free model and audit theorem-A quantities.
    # ------------------------------------------------------------------
    all_losses: list[list[float]] = []
    all_weight_norms: list[list[list[float]]] = []
    run_data: dict[int, dict[str, Any]] = {}

    print("=== Training dimension-free linear-attention model with theorem-A audits ===")
    for L in lvals:
        print(f"\nL = {L}")
        bundle = train_model_dim_free_with_theorem_a(
            d=d,
            P_tr=args.p_train,
            P_test=args.p_test,
            B=args.batch,
    ...
    # ------------------------------------------------------------------
    # Run the original four-variable reduced theory for spectral data.
    # ------------------------------------------------------------------
    print("\n=== Running four-variable reduced spectral theory ===")
    ...
    for L in lvals:
        print(f"\nL = {L}")
        loss_i, ws_i = reduced_theory_four_var_linear_att_spec(
            spec,
            w_star,
            L,
            eta=args.lr / sqrt_d,
    ...
    th_exp = 5.0 * args.beta / (5.0 * args.beta + 2.0)
    ...
    arr_gamma_eff_norm = hist_array("gamma_eff_norm")
    arr_gamma_fact = hist_array("gamma_fact")

    # ------------------------------------------------------------------
    # Figure 1: theorem-A main figure in the dimension-free rotated regime.
    # ------------------------------------------------------------------
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))

    for i, L in enumerate(lvals):
        axs[0, 0].loglog(train_t, np.asarray(all_losses[i], dtype=np.float64), color=f"C{i}", label=rf"$L = {L}$")
    axs[0, 0].loglog(train_t, train_t ** (-th_exp), "--", color="red", label=rf"$t^{{-5\beta/(5\beta+2)}}$")
    axs[0, 0].set_xlabel(r"$t$")
    axs[0, 0].set_ylabel(r"$\mathcal{L}(t)$")
    axs[0, 0].set_title("Training loss with power-law exponent reference")
    axs[0, 0].legend()
    ...
    axs[1, 1].set_ylabel(r"$|\bar\gamma_{\rm eff} - \gamma_{\rm fact}| / |\bar\gamma_{\rm eff}|$")
    axs[1, 1].set_title(r"Scalar mismatch: normalized $\gamma_{\rm eff}$ vs. $\gamma_{\rm fact}$")

    fig.suptitle(
        r"Theorem A on dimension-free linear-attention dynamics (rotated Bernoulli spectral data)",
        fontsize=17,
    )
    ...
    axs2[0, 1].plot(audit_steps, arr_gamma_eff_norm[idx_deep], label=r"$\bar\gamma_{\rm eff}$")
    ...
    axs2[0, 1].set_title(rf"Deepest depth $L = {L_deep}$: scalar gamma trajectories")
    axs2[0, 1].legend()
    ...
    labels = [r"$W_x / \sqrt{2}$", r"$W_q$", r"$W_k$", r"$W_v / \sqrt{d}$"]
    ws_final = np.asarray(all_ws_th[idx_deep], dtype=np.float64)
    wn_final = np.asarray(all_weight_norms[idx_deep], dtype=np.float64)
    for i in range(4):
    ...
    fig2.suptitle(
        r"Theorem A robustness diagnostics for dimension-free linear-attention dynamics",
        fontsize=17,
    )
    ...
    fig3.suptitle(r"Direct function comparisons on the fixed rotated debug batch", fontsize=16)
    ...
    payload: dict[str, np.ndarray] = {
        ...
        "gamma_eff_norm": arr_gamma_eff_norm,
    ...
    np.savez(out.numpy("theorem_a_dim_free_dynamics_data"), **payload)
    print(f"Saved data to: {out.numpy('theorem_a_dim_free_dynamics_data')}")
