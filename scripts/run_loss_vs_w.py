...
from utils import loss_landscape, make_powerlaw_spec_and_wstar, parse_int_list
...
def main() -> None:
    ...
    plot_path = (
        Path(args.plot_path)
        if args.plot_path
        else PROJECT_ROOT / "outputs" / "loss_vs_w.png"
    )
    ...
    losses_path = (
        ...
        if args.losses_path
        else PROJECT_ROOT / "outputs" / "loss_vs_w_losses.npz"
    )
